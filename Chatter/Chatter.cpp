// Chatter.cpp : Defines the entry point for the console application.
//
...
DWORD WINAPI DataStart(LPVOID lpParam)
{
	...
	while (running)
	{
		...
		{
			PostThreadMessage(networkThreadId, WM_SEND_TO_ALL, 0, reinterpret_cast<LPARAM>(message));
		}
	}
	...
	while (running)
	{
		...
		while (PeekMessage(&msg, NULL, WM_USER, 0, PM_REMOVE) > 0)
		{
			...
			case WM_REGISTER_DATA_THREAD:
				...
				break;
			case WM_SEND_TO_ALL:
				cs->SendToAll(reinterpret_cast<char*>(msg.lParam), DEFAULT_BUFLEN);
			default:
				break;
			}
		}
